...
def main():
  ...
  empty_row_dist = 1000000-1
  empty_col_dist = 1000000-1

  expanded_rows = [(1-min(sum(row), 1)) * empty_row_dist for row in space_grid]
  expanded_cols = [(1-min(sum([space_grid[j][i] for j in range(nrows)]), 1)) * empty_col_dist for i in range(ncols)]
